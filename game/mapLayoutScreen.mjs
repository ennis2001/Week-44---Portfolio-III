...
function createMapLayoutScreen() {
    const MapLayout = {
        ...
        draw: function (dr) {
            if (this.isDrawn) { return; } // We do not want to draw if there is no change.
            this.isDrawn = true;
            ...
            for (let y = 0; y < GAME_BOARD_DIM; y++) {
                ...
                for (let x = 0; x < GAME_BOARD_DIM; x++) {
                    ...
                    if (isInShipPreview && this.canPlaceShip()) {
                        // Show ship preview in green
                        output += ANSI.COLOR.GREEN + '█' + ANSI.RESET + ' ';
                    } else if (isInShipPreview) {
                        // Show ship preview in white if it can't be placed
                        output += ANSI.COLOR.WHITE + '█' + ANSI.RESET + ' ';
                    } else if (cell !== 0) {
                        // Show placed ships in the specified sea color
